...
cl::list<Searcher::CoreSearchType> CoreSearch(
    ...
    cl::values(
        ...
        clEnumValN(Searcher::AutoFeat, "auto", "use automated feature searcher")
...
cl::opt<std::string> FeatureCond(
		"feature",
		cl::desc("feature file for AutoFeature searcher"),
		cl::cat(SearchCat));

cl::opt<std::string> Weight(
		"weight",
		cl::desc("Weight file for AutoFeature searcher"),
		cl::cat(SearchCat));
} // namespace
...
bool klee::userSearcherRequiresMD2U() {
  return (std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::NURS_MD2U) != CoreSearch.end() ||
    ...
    std::find(CoreSearch.begin(), CoreSearch.end(), Searcher::AutoFeat) != CoreSearch.end());
}

Searcher *getNewSearcher(Searcher::CoreSearchType type, Executor &executor, const std::string &weightFile, const std::string &featureFile) {
  ...
  case Searcher::AutoFeat: searcher = new AutoFeatureSearcher(weightFile, featureFile, executor); break;
  }
...
Searcher *klee::constructUserSearcher(Executor &executor) {
  ...
  std::string weightFile, featureFile;
  if(!FeatureCond.empty())
    featureFile = FeatureCond;
  else
    featureFile = "";
  ...
  if (!Weight.empty())
    ...
    weightFile = "";
  Searcher *searcher = getNewSearcher(CoreSearch[0], executor, weightFile, featureFile);
  ...
  if (CoreSearch.size() > 1) {
    ...
    for (unsigned i = 1; i < CoreSearch.size(); i++)
      s.push_back(getNewSearcher(CoreSearch[i], executor, weightFile, featureFile));
